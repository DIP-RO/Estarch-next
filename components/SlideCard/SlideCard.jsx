...
const SlideCard = () => {
    ...
    return (
        <div className={`w-[100%] md:w-[30%] bg-base-100 shadow-2xl h-full z-[99999] p-6 fixed top-0 transition-all duration-500 ${isOpen ? "right-0" : "right-[-620px] hidden"}`}>
            ...
            {cartItems.length > 0 ? (
                <div className="relative h-full flex flex-col justify-between">
                    <div className="h-[70%] overflow-y-auto">
                        ...
                        {cartItems.map((item) => (
                            <div key={`${item.id}-${item.size}`} className="flex relative gap-4 py-5 border-b">
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-4 bg-white">
                        ...
                        <Link href="/product/order">
                            <button onClick={handleSlideCard} className="btn bg-black text-white w-full hover:bg-black">Place Order</button>
                        </Link>
                    </div>
                </div>
            ) : (
